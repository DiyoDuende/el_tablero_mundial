...
const UIPanelInfo = {
            ...
            case 'economia':
                contenido = `
                    <div class="dashboard-seccion">
                        ...
                        <p><strong>PIB:</strong> <span id="dato-pib">Cargando...</span></p>
                        <p><strong>Inflación:</strong> <span id="dato-inflacion">Cargando...</span></p>
                        <p><strong>Deuda/PIB:</strong> <span id="dato-deuda">Cargando...</span></p>
                        <p><strong>Desempleo:</strong> <span id="dato-desempleo">Cargando...</span></p>
    ...
    async cargarDatosEconomicosReales() {
        if (!window.DatosReales) return;
        
        const pibData = await DatosReales.obtenerPIBPerCapita();
        const nombrePais = this.normalizarNombrePais(this.paisActual);
        const pib = pibData[nombrePais];
        
        const pibSpan = document.getElementById('dato-pib');
        if (pibSpan) {
            if (pib) {
                pibSpan.textContent = new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'USD' }).format(pib);
            } else {
                pibSpan.textContent = 'Dato no disponible';
            }
        }
    },
    
    normalizarNombrePais(paisId) {
        const mapa = {
            'espana': 'Spain',
            'francia': 'France',
            'alemania': 'Germany',
            ...
            <div class="mas-info-contenido">
                <div class="mas-info-panel active" data-panel="economia">
                    <table class="info-detalle-tabla">
                        <tr><td>PIB (nominal):</td>
                        <td>${(info.economia.pib).toLocaleString()} M€</td>
                    </tr>
                    <tr>
                        <td>PIB per cápita:</td>
                        <td>${info.economia.pib_percapita.toLocaleString()} €</td>
                    </tr>
                    <tr>
                        <td>Inflación (IPC):</td>
                        <td>${info.economia.inflacion}%</td>
                    </tr>
                    <tr>
                        <td>Deuda pública (% PIB):</td>
                        <td>${info.economia.deuda}%</td>
                    </tr>
                    <tr>
                        <td>Tasa de paro:</td>
                        <td>${info.economia.desempleo}%</td>
                    </tr>
                    <tr>
                        <td>Balanza comercial:</td>
                        <td>${info.economia.balanza_comercial}% del PIB</td>
                    </tr>
                    </table>
                </div>
                <div class="mas-info-panel" data-panel="demografia">
                    <table class="info-detalle-tabla">
                        <tr>
                            <td>Población:</td>
                            <td>${info.demografia.poblacion.toLocaleString()} hab.</td>
                        </tr>
                        <tr>
                            <td>Densidad:</td>
                            <td>${info.demografia.densidad} hab/km²</td>
                        </tr>
                        <tr>
                            <td>Esperanza de vida:</td>
                            <td>${info.demografia.esperanza_vida} años</td>
                        </tr>
                        <tr>
                            <td>Población urbana:</td>
                            <td>${info.demografia.urbanizacion}%</td>
                        </tr>
                    </table>
                </div>
                <div class="mas-info-panel" data-panel="energia">
                    <table class="info-detalle-tabla">
                        <tr>
                            ...
                            <td>${info.energia.consumo_electrico} TWh/año</td>
                        </tr>
                        <tr>
                            <td>Renovables:</td>
                            <td>${info.energia.renovables}% del mix</td>
                        </tr>
                        <tr>
                            <td>Dependencia energética:</td>
                            <td>${info.energia.dependencia_exterior}%</td>
                        </tr>
                    </table>
                </div>
