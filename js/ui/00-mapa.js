const MapaMundial = {
    ...
    init: function() {
        ...
        this.cargarGeoJSON();
        setTimeout(() => this.irAPais('españa'), 500);
    },
    cargarMarcadores: function() {
        const paises = [
            { name: 'España', coords: [40.4,-3.7] }, { name: 'Francia', coords: [46.6,2.4] },
            { name: 'Portugal', coords: [39.5,-8.0] }, { name: 'Alemania', coords: [51.1,10.5] },
            { name: 'Italia', coords: [41.9,12.5] }, { name: 'EEUU', coords: [39.8,-98.5] }
        ];
        paises.forEach(p => {
            const marker = L.circleMarker(p.coords, { radius: 8, color: '#4fc3f7', fillColor: '#2a7faa', fillOpacity: 0.8 })
                .addTo(this.map)
                .bindTooltip(p.name)
                .on('click', () => this.irAPais(p.name.toLowerCase()));
        });
    },
    ...
    normalizarId: function(nombre) {
        const mapa = { Spain: 'españa', France: 'francia', Portugal: 'portugal', Germany: 'alemania', Italy: 'italia', 'United States': 'eeuu' };
        return mapa[nombre] || nombre.toLowerCase();
    },
    irAPais: function(paisId) {
        const centros = { españa: [40.4,-3.7], francia: [46.6,2.4], portugal: [39.5,-8.0], alemania: [51.1,10.5], italia: [41.9,12.5], eeuu: [39.8,-98.5] };
    ...
    activarCapa: async function(capa, activa) {
        ...
        if (this.capaActiva === capa && activa) return this.resetearColores();
        if (!activa) return this.resetearColores();
    ...
    obtenerValoresSimulados: function(capa) {
        const base = { Spain:0.7, France:0.8, Germany:0.9, Italy:0.6, Portugal:0.5, 'United States':0.9 };
        const valores = {};
    ...
    buscarLugar: function(texto) {
        ...
        .then(data => {
            ...
            if (data && data[0]) {
                ...
                L.marker([lat,lon]).addTo(this.map).bindTooltip(display_name).openTooltip().removeAfter(5000);
            } else alert('No encontrado');
        })
        .catch(() => { if (input) input.placeholder = orig; alert('Error de búsqueda'); });
    }
};
