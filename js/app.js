...
function cargarGeoJSON() {
    ...
    fetch(url)
        ...
        .catch(error => {
            ...
            document.getElementById("dashboard").innerHTML = `<div class="dashboard-real"><p>⚠️ Error al cargar el mapa</p></div>`;
        });
}

// Fallback para obtener ISO3 si no viene en el GeoJSON
...
async function cargarDatosBancoMundial(iso3, nombrePais) {
    ...
    try {
        // Consultar indicadores en paralelo
        const indicadores = [
        ...
        `;
        console.log(`✅ Datos cargados para ${nombrePais}`);
    } catch (error) {
        console.error("❌ Error en API:", error);
        dashboard.innerHTML = `<div class="dashboard-real"><div class="dashboard-header"><h3>❌ Error</h3></div><p>No se pudieron cargar los datos para ${nombrePais}</p></div>`;
    }
}
...
const datosVerificacion = {
    "subido el sueldo a los diputados": "❌ Falso. El sueldo base es 3.050,68€/mes. Subió un 2,5% (IPC).",
    "tropas españolas en ucrania": "❌ Falso. No hay tropas de combate. Sí hay instructores.",
    "iva de la luz": "❌ Falso. Sigue en el 10% (tipo reducido).",
    "sube el petróleo": "✅ Verdadero. Ha subido un 8% por tensiones en Oriente Medio."
};
...
function verificarAfirmacion() {
    ...
    for (let clave in datosVerificacion) {
        if (pregunta.includes(clave)) {
            respuesta = datosVerificacion[clave];
            break;
        }
    }
